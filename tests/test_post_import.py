...
def test_creates_person_type_purpose_foreign_key_when_no_orphans_exist() -> None:
    ...
    assert ensure_person_type_purpose_foreign_key(cursor) == "TABELLA_TIPO_PERSONA foreign key created"
    assert cursor.execute.call_count == 3
...
def test_does_not_create_existing_person_type_purpose_foreign_key() -> None:
    ...
    assert ensure_person_type_purpose_foreign_key(cursor) == "TABELLA_TIPO_PERSONA foreign key already exists"
    assert cursor.execute.call_count == 2
